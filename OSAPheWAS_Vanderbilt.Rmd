...
``` {r eval=TRUE}
#Read Data
#Note for race A = "Asian/Pacific", B = "Black", I = "Native American", M = "Mixed", N = "Other", U = "Unknown", W = "White"
...
#Above code is cleaner and below should produce the same as above....always good to check and useful if there are no fuzzy matches for grep
OSAICD9snums <- 
  Case.ICD9codes %>%
...
EHRdefined.OSAcases.Vanderbilt <- EHRdefined.OSAcases.Vanderbilt %>%
  select(GRID=study_id, gender, race) %>%
